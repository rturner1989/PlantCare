...
describe('Avatar', () => {
  describe('fallback', () => {
    it('renders the sprout fallback regardless of species personality', () => {
      const { container } = render(<Avatar species={species({ personality: 'dramatic' })} />)
      expect(container.textContent).toBe('🌱')
    })

    it('renders the sprout fallback when species is missing entirely', () => {
      const { container } = render(<Avatar species={null} />)
      expect(container.textContent).toBe('🌱')
    })
  })
  ...
  describe('appearance', () => {
    ...
    it('switches to rounded-full when shape="circle"', () => {
      const { container } = render(<Avatar species={species()} shape="circle" />)
      const el = container.firstChild
    ...
    it('forwards className and extra props to the root', () => {
      const { container } = render(
        <Avatar species={species()} className="border-2 border-card" data-testid="added-avatar" />,
      )
      const el = container.firstChild
  ...
  describe('image mode', () => {
    it('renders an <img> with object-cover when species.image_url is present', () => {
      const { container } = render(<Avatar species={species({ image_url: '/monty.jpg' })} />)
    ...
    it('falls back to the sprout emoji when no image_url', () => {
      const { container } = render(<Avatar species={species({ image_url: null })} />)
