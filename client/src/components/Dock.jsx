...
function DockNavLink({ to, label, icon }) {
  return (
    <NavLink to={to} end={to === '/'} className={({ isActive }) => `flex flex-col items-center gap-1 py-2 px-3 ${isActive ? 'text-forest' : 'text-ink-soft'}` }>
...
export default function Dock() {
  ...
  return (
    <nav className="fixed bottom-[10px] left-3 right-3 h-[74px] z-50 flex items-center justify-around px-4 lg:hidden bg-white/[0.78] backdrop-blur-xl backdrop-saturate-150 rounded-3xl border border-white/60 shadow-[var(--shadow-dock)]">
      ...
      <button type="button" onClick={() => navigate('/add-plant')} className="relative -top-3.5 flex items-center justify-center w-[54px] h-[54px] rounded-full text-white cursor-pointer border-0 bg-[image:var(--gradient-brand)] shadow-[var(--shadow-fab)]" >
